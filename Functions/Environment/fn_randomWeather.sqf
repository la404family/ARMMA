...
private _fnc_getWeather = {
    ...
    [_overcast, _fog, _rain, _windX, _windY, _windSpeed, _windAngle, _label]
};

private _initialWeather = call _fnc_getWeather;
_initialWeather params ["_overcast", "_fog", "_rain", "_windX", "_windY", "_windSpeed", "_windAngle", "_label"];
...
[_fnc_getWeather] spawn {
    params ["_fnc_getWeather"];
    private _transitionTime = 40 * 60; 

    while {true} do {

        private _nextWeather = call _fnc_getWeather;
        _nextWeather params ["_overcast", "_fog", "_rain", "_windX", "_windY", "_windSpeed", "_windAngle", "_label"];
